document.addEventListener("DOMContentLoaded", function () {
    ...
    reactionButton.addEventListener("click", () => {
        if (isReactTime) {
            ...
            reactionTimes.push({ genre: musicBPM, time: reactionTime });
            currentTrial++;
            startReactionTest();
        } else {
    ...
    function displayResults() {
        ...
        reactionTimes.forEach((entry) => {
            if (!timesByBPM[entry.genre]) {
                timesByBPM[entry.genre] = [];
            }
            timesByBPM[entry.genre].push(entry.time);
        });

        for (let genre in timesByBPM) {
            resultsList.innerHTML += `<h3>${genre} BPM</h3>`;
            timesByBPM[genre].forEach((time, index) => {
                resultsList.innerHTML += `<p>Trial ${index + 1}: ${time} ms</p>`;
            });
            const averageTime = Math.round(timesByBPM[genre].reduce((a, b) => a + b, 0) / timesByBPM[genre].length);
